...
namespace objloader
{
    ...
    bool Model::load(const std::string& data_path, const std::string& mesh_filename, const std::string& material_filename, Color4f colors)
    {
        dataPath_ = data_path;
        ...
        glColor4f(colors.red, colors.green, colors.blue, colors.alpha);
        glPopAttrib();
    ...
    void Model::processMeshLine(const std::string& line)
    {
        ...
        else if (line.find("vt ") == 0) // A texture coordinate
        {
            ...
            if (tokens.size() != 4) texture.coords[2] = 1.00;
            else texture.coords[2] = std::strtod(tokens.at(3).c_str(), 0);
